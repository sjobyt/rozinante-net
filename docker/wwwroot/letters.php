...
print_htmlheader("Reisebrev", $style, "");
...
if (($directory = opendir($letters))  && !($_GET['read'])) {
	...
	if($type == "info:") {
  		    ...
  		    setlocale ( LC_TIME, "no_NO");
     		    $pubtime = strftime("publisert: %d %B %Y %H:%M", filemtime($index_file));
...
if (substr($data, 0, 5) == "<?xml")
{
	...
	if ($name == "img")
	{
	    ...
	    if($img_attrs['link']) {
            ...
            insert_image($basedir . $img_attrs['src'], $charData, $link, $img_attrs['pos'], "",  $img_attrs['height']);
            $img_attrs['src'] = false;
            $img_comment = false;
	}
        else
